          ...
          <div className={styles.SectionBlogContainer}>
            <div className={styles.SectionBlogCard}>
              <div className={styles.SectionBlogText}>
                ...
                <p>
                When the Wesh app on the computer for <code>client2</code> sends the bytes of a message, what is the <code className="language-go">“flight path”</code> of the bytes of <code className="language-go">“Hello”</code> as they travel to the computer of <code>client1</code>? It&apos;s different from “traditional” networking where, for example, a computer sends the message in a UDP packet to the IP address of the other computer, as described in <a href="https://en.wikipedia.org/wiki/User_Datagram_Protocol" target="_blank">this Wikipedia article</a>. Instead of a specific IP address, Wesh communication happens between <a href="https://docs.libp2p.io/concepts/fundamentals/peers/" target="_blank">libp2p peers</a>, where the client&apos;s PeerID is the value that we&apos;ve seen in previous examples and is the same no matter how the client is connected to the network.
                </p>
                <h3>Peer discovery</h3>
                <p>
                The “glue” between a libp2p peers and a more traditional network address is <a href="https://docs.libp2p.io/concepts/discovery-routing/overview/" target="_blank">peer discovery</a>. There are a lot of details that we won&apos;t go into regarding types of discovery, peer authentication, etc. In this blog post, we just describe the minimal steps for how <code>client1</code> receives the message from <code>client2</code>. When the libp2p service of <code>client1</code> discovers the computer of <code>client2</code>, it opens a network connection (<a href="https://github.com/libp2p/go-libp2p-pubsub/blob/6d73cd4b56bf9d7fdd0c0caaed629a3d51027a75/comm.go#L115" target="_blank">code</a>) and reads from it using a traditional byte stream (<a href="https://github.com/libp2p/go-libp2p-pubsub/blob/6d73cd4b56bf9d7fdd0c0caaed629a3d51027a75/comm.go#L66">code</a>). (We give links to code for curious readers, but this isn&apos;t a code tutorial and you can skip the code links.)
                ...
                &nbsp;a pubsub message to this topic that contains the message log head. The libp2p on <code>client1</code> receives this pubsub message and sends it (<a href="https://github.com/libp2p/go-libp2p-pubsub/blob/6d73cd4b56bf9d7fdd0c0caaed629a3d51027a75/pubsub.go#L978" target="_blank">code</a>) internally to all processes which are subscribed to the topic. The Wesh application of <code>client1</code> has used ActivateGroup and is therefore subscribed to this topic and receives the pubsub message (<a href="https://github.com/berty/go-orbit-db/blob/368618e06b3be3bd17952d167d0e2f4fa03960ff/pubsub/pubsubraw/pubsub.go#L76" target="_blank">code</a>).
                </p>
                ...
                <p>
                The message store receives (<a href="https://github.com/berty/go-orbit-db/blob/368618e06b3be3bd17952d167d0e2f4fa03960ff/baseorbitdb/orbitdb.go#L828" target="_blank">code</a>) the event for a new head which contains the <code>CID</code> of the message. The message (which may be large) is fetched with IPFS, not with libp2p pubsub (which is better for small messages). If you don&apos;t know what a <code>CID</code> is and how IPFS uses it to fetch content, then you can <a href="https://docs.ipfs.tech/concepts/content-addressing/" target="_blank">read all about it</a>. Notice that using a <code>CID</code> to fetch the message (<a href="https://github.com/berty/go-ipfs-log/blob/11feb569ceaca84229987a039e102d799af9ccaa/entry/entry.go#L486" target="_blank">code</a>) like “Hello” is a separate process from synchronizing heads. Maybe IPFS has already fetched the same <code>CID</code>, or maybe it can be fetched from another group member that is not <code>client2</code>. Wesh takes advantage of the efficient IPFS algorithms.
                </p>
                <h3>Decrypting and finishing</h3>
                <p>
                We&apos;re almost done! When the “Hello” message is fetched, it&apos;s placed in the local copy of the message log in the correct order. (Messages in the device&apos;s storage remain encrypted.) This internally sends the <code>EventReplicated</code> event which is received (<a href="https://github.com/berty/weshnet/blob/aa44f7d64dcbc54c643cfddfaac8aa0a4d80f6a0/store_message.go#L452" target="_blank">code</a>) by the Wesh message monitor. We started at the libp2p “level” which doesn&apos;t care about the meaning of pubsub messages, up through the message log level which only cares about the correct order of messages. Now at the “top” level, Wesh must use <code>client1&apos;s</code> cryptographic keys to decrypt (<a href="https://github.com/berty/weshnet/blob/aa44f7d64dcbc54c643cfddfaac8aa0a4d80f6a0/store_message.go#L139" target="_blank">code</a>) the message.
                </p>
                <p>
                The decrypted message is sent internally (<a href="https://github.com/berty/weshnet/blob/aa44f7d64dcbc54c643cfddfaac8aa0a4d80f6a0/store_message.go#L195" target="_blank">code</a>) to all monitoring processes, including the <code>GroupMessageList</code> service (<a href="https://github.com/berty/weshnet/blob/aa44f7d64dcbc54c643cfddfaac8aa0a4d80f6a0/api_event.go#L191" target="_blank">code</a>). The <code>client1</code> application has called <code>GroupMessageList</code>, so the “Hello” message is sent (<a href="https://github.com/berty/weshnet/blob/aa44f7d64dcbc54c643cfddfaac8aa0a4d80f6a0/api_event.go#L199" target="_blank">code</a>) to the application which prints the message.
                </p>
                <h3>Flight of a Byte</h3>
                <p>
                In summary, the flight of a message from <code>client2</code> to <code>client1</code> starts when the <code>client2</code> peer is discovered, subscribes to the libp2p pubsub topic for the group messages and publishes its message log head. <code>client1</code> doesn&apos;t have this head yet, so it uses the CID to fetch the message using IPFS. When the message arrives, it is added to <code>client1&apos;s</code> copy of the message log in the correct order. Wesh sends this new message to processes using the <code>GroupMessageList</code> service, such as the sample application which receives it and prints “Hello”. Easy, right? Each of these steps is needed to make sure communication can still happen even if a peer disconnects or reconnects at a different network address.
                </p>
                <p>
